...
@dataclass
class BoundingBox:
    """Represents the rectangular bounds of an item in canvas coordinates."""
    left: int
    top: int
    right: int
    bottom: int
...
def clamped_delta(
    ...
    """Clamp a movement delta so the given bounding box (of one or more widgets) remains within canvas bounds."""
...
def nearest_in_bounds_grid_step(
    ...
    """Return the value of the nearest grid step of the given grid size that lies within the allowed range."""
    if grid_size <= 0:
...
def compute_widget_bounding_box(
    x: int,
    y: int,
    width: int,
    height: int,
    ...
    """Return a widget's bounding box based on its position, size and anchor."""
    if anchor == "sw":
